// Vista de Preferencias del Paciente (Notificaciones, Tema/Idioma, Seguridad y Eliminación de cuenta)
...
import { getSettings, updateSettings } from '../../services/patient.service.js';
import { changePassword, deleteAccount } from '../../services/auth.service.js';
import { router } from '../../router/router.js';

const BREADCRUMBS = [{ label: 'Panel', href: ROUTES.PATIENT_DASHBOARD }, { label: 'Preferencias' }];
  ...
  <div class="flex flex-col gap-6">
        ...
        content: `
          ...
          <div class="flex flex-col divide-y divide-black/5">
            ${ToggleRow({
              ...
              description: 'Recibe avisos sobre citas, tratamientos y seguimiento dentro de Palm Health.',
    ...
    ${Card({
      content: `
        ...
        <p class="mb-4 text-sm text-muted">Actualiza tu contraseña de acceso.</p>
    ...
    ${Card({
      ...
      content: `
        <h2 class="mb-1 text-base font-semibold text-error">Eliminar cuenta</h2>
        <p class="mb-4 text-sm text-muted">Esta acción desactivará tu cuenta. No podrás acceder al sistema a menos que contactes con soporte.</p>
...
export const mount = () => {
  ...
  const wireForms = () => {
    ...
    document.getElementById('delete-account-button')?.addEventListener('click', async () => {
      if (!confirm('¿Estás seguro de que deseas eliminar tu cuenta de Palm Health? Esta acción no se puede deshacer.')) {
        return;
      }
      try {
