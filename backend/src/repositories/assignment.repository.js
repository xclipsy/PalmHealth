...
class AssignmentRepository extends BaseRepository {
  ...
  async isProfessionalAssignedToPatient(professionalId, patientId) {
    const result = await this.execute(
      `SELECT 1
        ...
        WHERE ppa.professional_id = $1
          AND ppa.patient_id = $2
          AND ppa.status = $3
  ...
  // Elimina/finaliza todas las vinculaciones activas de un paciente.
  async softDeleteAllByPatientId(patientId) {
    await this.execute(
      `UPDATE patient_professional_assignments
          SET status = 'COMPLETED', deleted_at = NOW(), updated_at = NOW()
