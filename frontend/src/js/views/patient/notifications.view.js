/**
 * Patient notifications — list, unread indicator, mark as read,
 * delete and type filter (Part 4).
 *
 * API:
 *   GET    /api/patient/notifications (pagination, unreadOnly)
 *   PATCH  /api/patient/notifications/:id/read
 *   DELETE /api/patient/notifications/:id
 *
 * Type filtering is local (the API filters by unread only), applied
 * over the current page as allowed by the spec ("local search where
 * appropriate").
 */
...
import { Card, Pagination, NotificationBadge } from '../../components/ui.components.js';
...
import { listNotifications, markNotificationRead, deleteNotification } from '../../services/patient.service.js';

const BREADCRUMBS = [{ label: 'Panel', href: ROUTES.PATIENT_DASHBOARD }, { label: 'Notificaciones' }];
const PAGE_SIZE = 10;

/** One notification row. */
const NotificationItem = (notification) => `
  <li class="flex flex-col gap-2 py-4 first:pt-0 last:pb-0 ${notification.is_read ? '' : 'relative'}">
    ...
    content: `
      <div class="flex flex-col gap-6">
        <div>
          ...
          <p class="mt-1 text-sm leading-relaxed text-muted">Avisos sobre tus citas, tratamientos y seguimiento.</p>
        </div>
...
export const mount = () => {
  ...
  const load = async () => {
    ...
    try {
      ...
      // Refresca la campana de notificaciones
      listNotifications({ unreadOnly: 'true', limit: 1 })
        .then(({ pagination: p }) => {
          const slot = document.getElementById('bell-badge-slot');
          if (slot && p) slot.innerHTML = NotificationBadge({ count: p.totalItems ?? p.total ?? 0 });
        })
        .catch(() => {});

      // Local type filter over the current page.
      ...
      if (!rows.length) {
        listNode.innerHTML = EmptyState({
          ...
          description: state.unreadOnly || state.type
            ...
            : 'Cuando haya novedades sobre tu salud, las verás aquí.',
        });
        return;
      }
      ...
      listNode.querySelectorAll('[data-read]').forEach((button) => {
        button.addEventListener('click', async () => {
          try {
            await markNotificationRead(Number(button.dataset.read));
      ...
      listNode.querySelectorAll('[data-remove]').forEach((button) => {
        button.addEventListener('click', async () => {
          try {
            await deleteNotification(Number(button.dataset.remove));
