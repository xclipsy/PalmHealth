...
export const mount = () => {
  ...
  Promise.all([
    ...
    .then(([dashboard, upcoming, weekly, notifications]) => {
      ...
      todayNode.innerHTML = todayAppointments.items.length
        ? Card({ content: `<ul class="flex flex-col divide-y divide-black/5">${todayAppointments.items.map(AgendaRow).join('')}</ul>` })
