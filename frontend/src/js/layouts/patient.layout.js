/**
 * Patient layout — the reusable authenticated shell for every
 * /patient/* view (Part 4 navigation).
 *
 * Structure: fixed sidebar (desktop) / slide-in drawer (mobile),
 * topbar with breadcrumbs + notifications bell + user menu, content
 * area and a compact footer.
 *
 * Views call PatientLayout({ active, breadcrumbs, content }) for the
 * HTML and mountPatientLayout() to wire the interactive parts.
 */
...
import { listNotifications } from '../services/patient.service.js';

/** Sidebar navigation model: id must match the view's `active` key. */
const NAV_ITEMS = [
  { id: 'dashboard', label: 'Inicio', href: ROUTES.PATIENT_DASHBOARD, icon: 'M3 12l9-9 9 9M5 10v10h5v-6h4v6h5V10' },
  { id: 'symptoms', label: 'Diario de síntomas', href: ROUTES.PATIENT_SYMPTOMS, icon: 'M12 4v16m8-8H4' },
  { id: 'appointments', label: 'Citas', href: ROUTES.PATIENT_APPOINTMENTS, icon: 'M8 2v4M16 2v4M3 9h18M5 5h14a2 2 0 012 2v12a2 2 0 01-2 2H5a2 2 0 01-2-2V7a2 2 0 012-2z' },
  { id: 'treatments', label: 'Tratamientos', href: ROUTES.PATIENT_TREATMENTS, icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2' },
  { id: 'medications', label: 'Medicación', href: ROUTES.PATIENT_MEDICATIONS, icon: 'M19.5 12.5l-7 7a4.95 4.95 0 01-7-7l7-7a4.95 4.95 0 017 7zM8.5 8.5l7 7' },
  { id: 'routines', label: 'Rutinas', href: ROUTES.PATIENT_ROUTINES, icon: 'M13 10V3L4 14h7v7l9-11h-7z' },
  { id: 'observations', label: 'Observaciones', href: ROUTES.PATIENT_OBSERVATIONS, icon: 'M15 12a3 3 0 11-6 0 3 3 0 016 0zM2.5 12S6 5.5 12 5.5 21.5 12 21.5 12 18 18.5 12 18.5 2.5 12 2.5 12z' },
];

/** Secondary nav (below divider). */
const SECONDARY_ITEMS = [
  { id: 'profile', label: 'Mi perfil', href: ROUTES.PATIENT_PROFILE, icon: 'M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z' },
  { id: 'settings', label: 'Preferencias', href: ROUTES.PATIENT_SETTINGS, icon: 'M10.325 4.317a1.724 1.724 0 013.35 0 1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572 1.724 1.724 0 010 3.35 1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065 1.724 1.724 0 01-3.35 0 1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572 1.724 1.724 0 010-3.35 1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z M15 12a3 3 0 11-6 0 3 3 0 016 0z' },
];
...
const SidebarContent = (active) => `
  <nav class="flex flex-1 flex-col gap-1" aria-label="Navegación del paciente">
 ...
 * @returns {string}
 */
export const PatientLayout = ({ active, breadcrumbs, content }) => {
  const { user } = getState();
  const fullName = `${user?.firstName || ''} ${user?.lastName || ''}`.trim() || 'Paciente';
  ...
  return `
    <div class="flex min-h-screen bg-background">
      ...
      <aside class="sticky top-0 hidden h-screen w-64 shrink-0 flex-col gap-6 border-r border-black/5 bg-surface px-4 py-6 lg:flex" aria-label="Barra lateral">
        <a href="${ROUTES.PATIENT_DASHBOARD}" data-link class="flex items-center gap-2 px-2" aria-label="Palm Health — Inicio del paciente">
        ...
        <p class="px-2 text-xs text-muted">Palm Health · Panel del paciente</p>
      </aside>
      ...
      <div id="mobile-drawer" class="fixed inset-0 z-40 hidden lg:hidden" role="dialog" aria-modal="true" aria-label="Menú de navegación">
        ...
        <aside class="absolute inset-y-0 left-0 flex w-72 flex-col gap-6 bg-surface px-4 py-6 shadow-xl">
      ...
      <div class="flex min-w-0 flex-1 flex-col">
        <header class="sticky top-0 z-30 border-b border-black/5 bg-surface/95 backdrop-blur">
          <div class="flex items-center justify-between gap-3 px-4 py-3 sm:px-6">
            ...
            <div class="flex items-center gap-2">
              <a href="${ROUTES.PATIENT_NOTIFICATIONS}" data-link id="notifications-bell"
              ...
              <div class="relative">
                ...
                <div id="user-menu" class="absolute right-0 top-full z-40 mt-2 hidden w-56 rounded-2xl bg-surface p-2 shadow-lg ring-1 ring-black/5" role="menu">
                  ...
                  <a href="${ROUTES.PATIENT_PROFILE}" data-link role="menuitem"
                    class="block rounded-xl px-3 py-2 text-sm text-foreground hover:bg-black/5">Mi perfil</a>
                  <a href="${ROUTES.PATIENT_SETTINGS}" data-link role="menuitem"
        ...
        <main id="main-content" class="mx-auto w-full max-w-5xl flex-1 px-4 py-6 sm:px-6 sm:py-8">${content}</main>

        <footer class="border-t border-black/5 px-6 py-4 text-center text-xs text-muted">
          Palm Health — Tu salud, organizada. Esta plataforma no sustituye la atención médica de urgencia.
        </footer>
      </div>
    </div>
  `;
 ...
 */
export const mountPatientLayout = () => {
  /* Mobile drawer */
  ...
  listNotifications({ unreadOnly: 'true', limit: 1 })
    .then(({ pagination }) => {
      ...
      if (slot && pagination) slot.innerHTML = NotificationBadge({ count: pagination.total });
    })
    .catch(() => {
